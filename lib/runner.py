...
class Runner:
    ...
    def eval(self, epoch, on_val=False, save_predictions=False):
        ...
        if on_val:
            ...
            dataloader = self.get_test_dataloader() # NoLabel gets here
        test_parameters = self.cfg.get_test_parameters()
        print(str(test_parameters))
        predictions = []
        self.exp.eval_start_callback(self.cfg)
        with torch.no_grad():
            tqdm_dataloader = tqdm(dataloader)
            print('*************RUNNING*********', len(tqdm_dataloader), len(dataloader))
            # images_array = [cv2.imread('C:/Users/jzhan/OneDrive/Documents/GitHub/LaneATT/datasets/MYDATA/img1.png')]
            # print('array', images_array)
            # for idx, images in enumerate(images_array):
            for idx, (images, _, _) in enumerate(tqdm_dataloader):
                images = images.to(self.device)
                ...
                if self.view:
                    ...
                    if self.view == 'mistakes' and fp == 0 and fn == 0:
                        continue
                    temp_folder_name = 'temp-2022-04-01_15-33-33-front.mp4'
                    outputdir = 'C:/Users/jzhan/OneDrive/Documents/GitHub/LaneATT/datasets/tesla-test/output-{}/'.format(temp_folder_name)
                    if not os.path.exists(outputdir):
                        os.makedirs(outputdir)
                    cv2.imwrite(outputdir + str(idx) + '.png', img)
                    # cv2.imshow('pred', img)
                    # cv2.waitKey(0)
